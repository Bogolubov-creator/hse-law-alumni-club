...
describe.skipIf(!enabled)("Поддержка: реальная БД, доступ и жизненный цикл",()=>{
 ...
 it("гость не получает переписку; офис и администратор читают и отвечают", async () => {
  ...
  try {
   ...
   for (const role of ["editor", "admin"] as const) {
    const headers = { authorization: "Bearer " + signAdmin(randomUUID(), role) };
    expect((await a.inject({ url: "/admin/support", headers })).statusCode).toBe(200);
    expect((await a.inject({ method: "PATCH", url: "/admin/support/" + b.id, headers, payload: { message: "Ответ сотрудника " + role, status: "answered" } })).statusCode).toBe(200);
   }
   const r = await a.inject({ url: "/support/" + b.id, headers: { "x-support-key": b.key } });
   expect(r.json().messages.map((m: { text: string }) => m.text)).toEqual([b.message, "Ответ сотрудника editor", "Ответ сотрудника admin"]);
