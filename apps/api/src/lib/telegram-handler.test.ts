...
it.each(["group", "supergroup", "channel"])("не читает профиль и не отвечает в %s", async type => {
  ...
  for (const text of ["/points", "/start " + makeTgLinkCode(owner), "/calendar", "как вступить"]) await handleTelegramUpdate(update(text, type), "test-token");
  expect(read).not.toHaveBeenCalled(); expect(send).not.toHaveBeenCalled(); expect(answerTelegramFaq).not.toHaveBeenCalled();
});
it("привязка экранирует имя и переносит Telegram с прежней записи", async () => {
  resetDb({ alumni: [{ id: owner, fio: '<b>Иван & "тест"</b>', telegram_id: null }, { id: "previous", telegram_id: "42" }] });
  const reply = await buildBotReply("/start", makeTgLinkCode(owner), "42");
  expect(reply).toContain("&lt;b&gt;Иван &amp; &quot;тест&quot;&lt;/b&gt;");
  expect(db.alumni?.find(x => x.id === owner)?.telegram_id).toBe("42");
  expect(db.alumni?.find(x => x.id === "previous")?.telegram_id).toBeNull();
});
