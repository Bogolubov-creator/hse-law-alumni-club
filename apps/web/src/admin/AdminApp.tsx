...
function EventForm({ busy, onClose, onSave }: { busy: boolean; onClose: () => void; onSave: (v: { title: string; description?: string | null; starts_at: string; location?: string | null; format?: string; points?: number }) => void }) {
  const [f, setF] = useState({ title: "", description: "", date: "", time: "19:00", location: "", format: "offline", points: "60" });
  const set = (k: string, v: string) => setF((s) => ({ ...s, [k]: v }));
  const valid = f.title.trim().length >= 3 && /^\d{4}-\d{2}-\d{2}$/.test(f.date);
  const submit = (e: FormEvent) => {
    e.preventDefault();
    if (!valid) return;
    onSave({ title: f.title.trim(), description: f.description.trim() || null, starts_at: `${f.date}T${f.time || "19:00"}:00+03:00`, location: f.location.trim() || null, format: f.format, points: Number(f.points) || 60 });
  };
  return (
    <Modal onClose={onClose} labelledBy="ev-form-title" maxWidth={520}>
      <form onSubmit={submit} className="rounded-[18px] bg-white p-7">
        <h3 id="ev-form-title" className="font-display text-lg font-bold">Новое событие</h3>
