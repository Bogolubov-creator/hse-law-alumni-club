...
export default function ChangesPreview() {
  ...
  return <section className="changes-preview" aria-labelledby="changes-preview-title">
    ...
    {q.data && <p className="changes-muted">{q.data.lastSuccessAt ? `Загружено из канала: ${changeTime(q.data.lastSuccessAt)} · ${q.data.syncStatus === "unavailable" ? "источник временно недоступен" : "проверка новых публикаций каждый час"}` : "Архивные записи · ожидается обновление из канала"}</p>}
