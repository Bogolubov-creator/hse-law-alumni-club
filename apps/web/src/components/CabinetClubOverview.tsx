...
export function CabinetClubOverview({ me, token }: { me: Me; token: string }) {
  ...
  return <div className="cabinet-club-overview">
    ...
    <section className="cabinet-next-event" aria-labelledby="cabinet-event-title">
      <div className="cabinet-block-heading"><h2 id="cabinet-event-title">Ближайшая встреча</h2><Link className="foc" to="/events">Вся афиша</Link></div>
      {events.isLoading ? <p role="status">Загружаем встречи…</p> : events.isError ? <p>Не удалось загрузить афишу. <button className="foc" onClick={() => events.refetch()}>Повторить</button></p> : next ? <>
        ...
        <Link className="foc cabinet-event-action" to={`/events/${next.id}`}>{next.my_rsvp ? 'Вы записаны – детали встречи' : 'Подробнее и запись'}</Link>
    ...
    <section className="cabinet-latest-news" aria-labelledby="cabinet-news-title">
      <div className="cabinet-block-heading"><h2 id="cabinet-news-title">Новости клуба</h2><Link className="foc" to="/news">Все новости</Link></div>
      {news.isLoading ? <p role="status">Загружаем новости…</p> : news.isError ? <p>Не удалось загрузить новости. <button className="foc" onClick={()=>news.refetch()}>Повторить</button></p> : news.data?.length ? news.data.map(n=><article key={n.slug}><time>{formatNewsDate(n.published_at)}</time><h3><Link className="foc" to={`/news/${n.slug}`}>{n.title}</Link></h3>{n.excerpt && <p>{n.excerpt}</p>}</article>) : <p>Новых публикаций пока нет.</p>}
    </section>
