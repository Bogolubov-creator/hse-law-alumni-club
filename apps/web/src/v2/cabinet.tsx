...
export function CabinetShell({ active, onLogout, children }: { active: "lk" | "profile"; onLogout: () => void; children: ReactNode }) {
  ...
  return (
    <div className="cabinet-shell" style={{ background: "var(--c-bg)", color: "var(--c-text)", fontFamily: "var(--f-body)", minHeight: "100dvh" }}>
      ...
      <nav className="cabinet-club-nav desk-only" aria-label="Разделы клуба">{[["/", "Главная"], ["/news", "Новости"], ["/events", "События"], ["/dpo", "ДПО"], ["/merch", "Мерч"], ["/podcasts", "Подкасты"], ["/changes", "Изменения в праве"], ["/cart", "Корзина"], ["/support", "Поддержка"]].map(([to, title]) => {
