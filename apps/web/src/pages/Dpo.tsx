            ...
            <div key={p.id} className="flex flex-col overflow-hidden rounded-[18px] border border-[#E5E7EB] bg-white">
              ...
              <div className="flex flex-1 flex-col p-5">
                ...
                <div className="mt-auto pt-4">
                  ...
                  <button disabled={add.isPending} onClick={() => add.mutate({ type: "dpo", ref_id: p.slug }, { onSuccess: () => toast(`«${p.title}» в корзине`), onError: () => toast("Не удалось добавить", "err") })} className="foc mt-3 w-full rounded-[12px] bg-hse-blue py-3 font-semibold text-kost disabled:opacity-60">В корзину</button>
                </div>
              </div>
            </div>
          ))}
