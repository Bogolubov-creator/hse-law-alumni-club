...
export default function Merch() {
  ...
  return (
    <SiteShell>
      ...
      {open && (
        <ProductModal product={open} discount={discount} onClose={() => setOpen(null)} onAdd={(sku) => { add.mutate({ type: "merch", ref_id: open.slug, variant_sku: sku }); setOpen(null); }} />
      )}
    </SiteShell>
  );
}
...
function ProductModal({ product, discount, onClose, onAdd }: { product: Product; discount: number; onClose: () => void; onAdd: (sku: string | null) => void }) {
  ...
  return (
    <div onClick={onClose} className="fixed inset-0 z-[100] flex items-center justify-center bg-[rgba(15,18,24,.55)] p-6 backdrop-blur-sm">
      <div onClick={(e) => e.stopPropagation()} className="relative w-full max-w-[460px] rounded-[22px] bg-white p-8 shadow-2xl" style={{ animation: "g-pop .26s cubic-bezier(.2,.8,.2,1)" }}>
        ...
        <h2 className="mt-2 font-display text-2xl font-bold leading-tight tracking-tight">{product.title}</h2>
        ...
        {variants.length > 0 && (
          <div className="mt-5">
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {variants.map((v) => (
                <button key={v.sku} onClick={() => setSku(v.sku)} disabled={v.stock <= 0} className={`foc rounded-[10px] border px-3.5 py-2 text-sm font-medium disabled:opacity-40 ${sku === v.sku ? "border-ohra bg-ohra text-kost" : "border-[#E5E7EB] bg-white"}`}>{vLabel(v)}</button>
              ))}
            </div>
          </div>
        )}
