    ...
    <SiteShell>
      ...
      <div className="mx-auto max-w-[1180px] px-7 py-14">
        ...
        <div className="two-col mt-9 grid grid-cols-3 gap-6">
          {news.data?.map((n) => (
            <Link key={n.id} to={`/news/${n.slug}`} className="vcard foc block overflow-hidden rounded-[18px] border border-[#E5E7EB] bg-white">
              ...
              <div className="p-5">
                <div className="font-mono text-[11px] tracking-[0.05em] text-[#6B7280]">{formatNewsDate(n.published_at)}</div>
                <div className="mt-2.5 font-display text-[17px] font-semibold leading-tight tracking-tight">{n.title}</div>
                <p className="mt-2.5 text-sm leading-snug text-[#6B7280]">{n.excerpt}</p>
