...
export default function Stub({ title }: { title: string }) {
  ...
  // 404 общая для обеих версий: с /v2/несуществующее «На главную» должна вести
  // на главную той же версии, иначе пользователя молча перебрасывает в V1.
  const home = useLocation().pathname.startsWith("/v2") ? "/v2" : "/";
  return (
    <main className="mx-auto flex min-h-screen max-w-3xl flex-col items-start justify-center px-6">
      ...
      <Link to={home} className="foc mt-6 font-display text-sm font-bold text-ohra-deep">
        ← На главную
      </Link>
    </main>
  );
