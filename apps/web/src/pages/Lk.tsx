...
function DashboardBody({ me, token, onBadge }: { me: import("../lib/api.js").Me; token: string; onBadge: (a: Achievement) => void }) {
  ...
  return (
    <>
      <Events token={token} />
      <NewbieChecklist me={me} />
      <NextClubEvent token={token} />
      ...
      <div id="push-bell"><PushBell /></div>

      {/* TELEGRAM-БОТ: привязка для /points и /calendar */}
      <div id="tg-link"><TgLink /></div>
...
/** Ближайшее событие клуба прямо в ЛК: дата, место и «Пойду» без перехода на афишу. */
function NextClubEvent({ token }: { token: string }) {
  const t = useLkTokens();
  const surface = lkSurface(t);
  const toast = useToast();
  const qc = useQueryClient();
  interface Ev { id: string; title: string; starts_at: string; location: string | null; format: string; points: number; status: string; going: number; my_rsvp: boolean; my_attended: boolean }
  ...
  if (!ev) return null;
  const when = new Date(ev.starts_at).toLocaleString("ru-RU", { day: "numeric", month: "long", hour: "2-digit", minute: "2-digit" });
  return (
    <div style={{ ...surface, padding: "20px 28px", marginBottom: 22, display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap", borderLeft: "4px solid #2E6FAE" }}>
      <div style={{ flex: 1, minWidth: 220 }}>
        <div style={{ ...mono, fontSize: 11, letterSpacing: ".12em", textTransform: "uppercase", color: "#2E6FAE" }}>Ближайшее событие клуба</div>
        <div style={{ ...disp, fontWeight: 600, fontSize: 18, marginTop: 7, lineHeight: 1.25 }}>{ev.title}</div>
        <div style={{ ...mono, fontSize: 12, color: t.muted, marginTop: 6 }}>
          {when} · {ev.format === "online" ? "онлайн" : ev.location ?? "очно"}{ev.points > 0 ? ` · +${ev.points} баллов` : ""}{ev.going > 0 ? ` · пойдут: ${ev.going}` : ""}
        </div>
      </div>
      {ev.my_attended ? (
        <span style={{ ...mono, fontSize: 12, fontWeight: 600, color: "#1F8A5B", flex: "none" }}>посещение засчитано ✓</span>
      ) : (
        <button onClick={() => rsvp.mutate(ev.id)} disabled={rsvp.isPending}
          className="foc" style={{ fontWeight: 600, fontSize: 14, padding: "11px 20px", borderRadius: 12, flex: "none", cursor: "pointer", ...(ev.my_rsvp ? { border: "1.5px solid #1F8A5B", background: t.ghostBtnBg, color: "#1F8A5B" } : { border: "none", background: "#2E6FAE", color: "#FBF3E8" }) }}>
          {ev.my_rsvp ? "Иду ✓ (отменить)" : "Пойду"}
        </button>
      )}
