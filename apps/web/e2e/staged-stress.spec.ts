...
test('reflow, intermediate widths, landscape and images',async({page},info)=>{
 ...
 for(const width of [699,700,701,767,768,769,899,900,901,999,1000,1001,1100,1101,1920]){
  await page.setViewportSize({width,height:600});for(const route of ['/v2','/v2/merch']){await page.goto(route);await expect(page.locator('h1')).toBeVisible();expect(await page.evaluate(()=>document.documentElement.scrollWidth)).toBeLessThanOrEqual(width+1)}
 }
 for(const route of ['/v2','/v2/dpo','/v2/merch','/v2/lk','/v2/join']){
 ...
 await page.setViewportSize({width:844,height:390});await page.goto('/v2/merch');await page.getByRole('button',{name:'Выбрать размер',exact:true}).first().click();const modal=page.getByRole('dialog');await expect(modal).toBeVisible();await modal.getByRole('button',{name:'Закрыть',exact:true}).click({trial:true});await page.screenshot({path:`${out}/landscape-merch.png`});await page.keyboard.press('Escape');
 ...
 await page.setViewportSize({width:390,height:844});await page.goto('/v2/merch');await expect(page.locator('.club-merch-item').first().getByText('Фотография пока недоступна')).toBeVisible();await expect(page.locator('.club-merch-item').nth(1).locator('img')).toBeVisible();await page.screenshot({path:`${out}/broken-and-valid-image.png`,fullPage:true});
 await page.unroute('**/api/products');
 for(const route of ['/v2','/v2/dpo','/v2/merch','/v2/join']){
