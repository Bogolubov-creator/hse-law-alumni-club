...
test.describe("Новости v2", () => {
  ...
  test("публикация разбивает текст на абзацы, а не сливает в один", async ({ page }) => {
    ...
    await page.goto("/news/test-post");
  ...
  test("несуществующая публикация объясняет это и не индексируется", async ({ page }) => {
    await page.goto("/news/takoj-novosti-net");
...
test.describe("События v2", () => {
  ...
  test("гостю предлагают вступить, а не молча ничего не делают", async ({ page }) => {
    ...
    const join = page.getByRole("link", { name: "вступить, чтобы записаться" }).first();
...
test.describe("Подкасты v2", () => {
  ...
  test("на телефоне разделы не едут вбок", async ({ page }) => {
    ...
    for (const url of ["/news", "/events", "/podcasts"]) {
      await page.goto(url);
...
test("в шапке канона нет ссылок на legacy-фронт", async ({ page }) => {
  ...
  const legacy = hrefs.filter((h) => /^\/legacy(\/|$)/.test(h));
  expect(legacy, `шапка ведёт в legacy: ${legacy.join(", ")}`).toEqual([]);
});
