...
Корзина хранится в `sessionStorage` текущей вкладки и переживает её перезагрузку. Это только просмотр состава и суммы: отправка заявки отключена. Правки профиля, контента и записей на события возвращают сообщение о недоступности сохранения, без ложного подтверждения успеха.

Результаты последней проверки интерфейса: [11 сентября 2026](frontend-release-review-2026-09-11.md).

## Обновление правовой библиотеки

С 22.09.2026 workflow зеркала проверяет открытую ленту LegisDigest каждый час (17-я минута UTC) и перед сборкой обновляет `data/changes.json`. Сохраняется предыдущая опубликованная база. GitHub может задерживать плановые задания; страница показывает фактическое время успешного импорта отдельно от последнего поста. Подробности и ручной запуск: [интеграция LegisDigest](legis-digest-integration.md).
