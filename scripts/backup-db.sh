...
if [ -z "${BACKUP_ENCRYPTION_KEY:-}" ]; then
  BACKUP_ENCRYPTION_KEY="$(grep '^BACKUP_ENCRYPTION_KEY=' "$ENV_FILE" | cut -d= -f2- || true)"
fi
if [ -z "${BACKUP_ENCRYPTION_KEY:-}" ]; then
...
if [ -n "${BACKUP_OFFSITE_REMOTE:-}" ]; then
  if command -v rclone >/dev/null 2>&1; then
    ...
    echo "OFFSITE OK: $BACKUP_OFFSITE_REMOTE"
  else
    echo "ОШИБКА: BACKUP_OFFSITE_REMOTE задан, но rclone не установлен – offsite-копия НЕ сделана" >&2
    exit 1
  fi
fi
