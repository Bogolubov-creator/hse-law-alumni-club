...
- 2026-06-30 Отложенные пункты — поэтапно (5 этапов, каждый отдельным коммитом):
  1. a11y: контраст мелкого текста `#9aa0aa → #6B7280` (WCAG AA).
  2. a11y: явная связь `label↔input` (`htmlFor`/`id`, `useId`) в формах Cart/Profile/ЛК/админки.
  3. Безопасность: отдельный опц. `ADMIN_AUTH_SECRET` + TTL админ-сессии 12ч (alumni остаётся 7д);
     типизация JWT-payload (убран `as any` в verify).
  4. Типобезопасность: рантайм-валидация ответов API через zod-схемы `@club/shared` (`responses.ts`,
     `z.infer` = единый источник типов фронта); `apiGet/apiPost/cartFetch/submitOrder` парсят ответ.
     Проверено вживую: витрины/новости/ЛК/«Мои заявки» проходят валидацию, консоль чистая.
  5. Типизированная Directus-`Schema` (строки коллекций) — `as any` убран со ВСЕХ чтений в
     engine/cart/orders/me/admin/auth/content/points; остались 4 намеренных каста (datetime-фильтр decay,
     реляц. `achievement_id.key`, `role.name`, M2A-глубокая выборка pages) + записи createItem/updateItem
     (генерики input SDK капризны) + bootstrap-скрипт (инструмент создания схемы). Сборка/тесты зелёные.
