...
-- scripts/apply-indexes.sh включает ON_ERROR_STOP: ошибка останавливает применение.
-- Перед применением сверять фактические индексы и ограничения конкретной БД.
...
-- Историческая пара определений: при наличии unique первый индекс избыточен.
-- Сохраняем до проверки схемы рабочего сервера; удаление выполняется отдельной миграцией.
